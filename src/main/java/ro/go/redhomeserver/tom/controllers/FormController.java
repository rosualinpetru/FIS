...
public class FormController {
    ...
    public List<Pair<String, String>> getEmployeesOfDepartment(@RequestParam("departmentId") String departmentId, Authentication authentication) {
        return formService.loadAccountsOfDepartmentByIdExceptMe(departmentId, authentication.getName()).stream().map(s -> new Pair<>(s.getEmployee().getId(), s.getEmployee().getName())).collect(Collectors.toList());
    }
    ...
    public List<Pair<String, String>> getEmployeesOfDepartment(@RequestParam("departmentId") String departmentId) {
        return formService.loadAccountsOfDepartmentById(departmentId).stream().map(s -> new Pair<>(s.getEmployee().getId(), s.getEmployee().getName())).collect(Collectors.toList());
    }
}
